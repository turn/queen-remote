...
var ClientQueen = function(socket, queen){
	...
	this.workerProviderDeadHandler = this.workerProviderDeadHandler.bind(this);
	...
	queen.on('workerProviderDead', this.workerProviderDeadHandler);
...
ClientQueen.prototype.workforceMessageHandler = function(workforceId, message){
	var workforce = this.workforces[workforceId];

	if(workforce !== void 0){
		workforce(message);
	} else {
		this.log("Workforce doesn't exist: " + JSON.stringify(workforceId));
	}
};
...
ClientQueen.prototype.workerProviderDeadHandler = function(queenWorkerProvider){
